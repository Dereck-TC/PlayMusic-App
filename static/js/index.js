 ...
 */
 const Sphere = {
    ...
    update(box, mouse, freq) {
     ...
     this.move.x = box.width * 0.06 + -(mouse.x * 0.02);
   ...
   new Vue({
    ...
    methods: {
     ...
     getChannels(sidebar) {
      ...
      let emsg = [
       "There was a problem trying to load the list of available channels from SomaFM."
      ];
      ...
      axios
       ...
       .then((res) => {
        if (!res || !res.data || !res.data.channels) {
         emsg.push(
          "The API response did not have any channels data available at this time."
         );
        ...
        for (let c of res.data.channels) {
         ...
         c.plsfile =
          c.playlists
           .filter((p) => p.format === "mp3" && /^(highest|high)$/.test(p.quality))
           .shift().url || "";
     ...
     fetchSongs(channel, cb) {
      ...
      let emsg = [
       "There was a problem trying to load the list of songs for channel " +
        channel.title +
        " from SomaFM."
      ];
   
      axios
       .get(channel.songsurl)
       .then((res) => {
        if (!res || !res.data || !res.data.songs) {
         emsg.push(
          "The API response did not have any songs data available at this time."
         );
     ...
     setupCanvas() {
      ...
      this.fxRenderer = new THREE.WebGLRenderer({
       alpha: true,
       antialias: true,
       precision: "highp"
      });
      ...
      // setup camera
      this.fxCamera = new THREE.PerspectiveCamera(
       60,
       this.fxBox.width / this.fxBox.height,
       0.1,
       20000
      );
      ...
      this.fxColor.setHSL(this.fxHue, 1, 0.5);
      // setup light source
     ...
     updateMousePosition(e) {
      ...
      this.fxMouse.x =
       Math.max(0, e.pageX || e.clientX || 0) -
       (this.fxBox.left + this.fxBox.width / 2);
      this.fxMouse.y =
       Math.max(0, e.pageY || e.clientY || 0) -
       (this.fxBox.top + this.fxBox.height / 2);
     },
     ...
     updateStageLight() {
      ...
      this.fxColor.setHSL(color, 0.5, 0.5);
     },
     ...
     updateAnimations() {
      ...
      if (!this.fxRenderer || !this.fxCamera || !this.analyser || !this.freqData)
       return;
     ...
     setupAudio() {
      ...
      this.audio.addEventListener("error", (e) => {
       let emsg = [];
       emsg.push(
        "The selected audio stream could not load, or has stopped loading."
       );
       emsg.push("Try again, or check your internet connection.");
